...
async function binarySearch(array, val) {
    ...
    while (low <= high) {
        await waitcount(delay)
        let mid = Math.floor((low + high) / 2)
        if (array[mid].element.innerHTML == val) {
            array[mid].element.style.background = 'green'
            array[mid].element.style.color = '#fcfcfc'
            findedAudio.play()
            findingAudio.pause()
            count++
            console.log(count)
            const step = document.querySelector('.step')
            step.innerHTML = `${count}`
            return array[mid].originalIndex;
        }
        if (val > array[mid].element.innerHTML) {
            array[mid].element.style.background = 'red'
            array[mid].element.style.color = 'white'
            findingAudio.play()
            count++
            low = mid + 1;
        } else {
...
async function sorting(array) {
    array.sort((a, b) => {
        return a.element.innerHTML - b.element.innerHTML
    })
    return array
}
...
async function Arrange(array) {
    ...
    for (let i = 0; i < array.length; i++) {
        body.appendChild(array[i].element)
    }
}
...
const binary = document.querySelector('#binary_Search').addEventListener('click', async () => {
    ...
    array1.forEach((element, index) => {
        Array.push({ element, originalIndex: index })
    })
    ...
    if (val != '') {
        ...
        disableSizeSlider();
        disableNewArrayBtn();
        var ind = await binarySearch(Array, parseInt(val))
        const index = document.querySelector('.index')
        if (ind != -1) {
            searchText.innerHTML = `Searching Complete`
            index.innerHTML = `${val} is present at original index no. ${ind}`
        } else {
            searchText.innerHTML = `Not Found!!`
            index.innerHTML = `${val} is not present in the array!!`
        }
